...
struct EmailLinkingView: View {
    @Binding var isPresented: Bool
    @StateObject private var viewModel = EmailLinkingViewModel()
    ...
    var body: some View {
                        ...
                        VStack(spacing: 24) {
                            ...
                            VStack(spacing: 16) {
                                ...
                                Text(viewModel.currentStep == .email ? 
                                     "Add an email to your account for easier sign in" : 
        ...
        .alert("Success", isPresented: $viewModel.showSuccess) {
            ...
            Text("Email account linked successfully!")
        }
...
@MainActor
final class EmailLinkingViewModel: ObservableObject {
    ...
    private let accountLinking = AccountLinkingService.shared
    ...
    func linkEmail() {
        ...
        Task {
            do {
                try await accountLinking.linkEmailAccount(
                    email: email,
                ...
                isLoading = false
                errorMessage = "Failed to link email. Please try again."
                showError = true
...
struct EmailLinkingView_Previews: PreviewProvider {
    static var previews: some View {
        EmailLinkingView(isPresented: .constant(true))
            .preferredColorScheme(.dark)
    }
}